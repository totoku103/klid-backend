...
$('#pbtnAdd').click(function() {
    ...
    var params={
        ...
        //사고 조사 내용, 시도의견
        inciDclCont: $("#inciDclCont").val().replace(/[\u0001-\u0008|\u000B-\u001F]/g,""),
    ...
    $.ajax({
        type: "post",
        url: $('#ctxPath').val() + '/api/main/acc/accidentApply/addAccidentApply',
        ...
        success : function(result) {
            ...
            $('#fileUpload').jqxFileUpload({
                uploadUrl : ctxPath + '/api/file/accUpload?inciNo=' + result.resultData
            });
                        ...
                        $.ajax({
                            ...
                            success:function (uploadResult){
                                if(uploadResult.result!="OK"){
                                    ...
                                    successAccidentAdd(result.resultData);
                                }
                            },
...
$('#checkDuplicateByDcl').click(function() {
    ...
    $.post(ctxPath + '/main/popup/comm/pAccDuplList.do', function(result) {
        HmWindow.open(pwin, '사고중복 확인', result, 700, 500, 'pAccDuplWindow_init', checkedValues);
    });
});

$('#checkDuplicateByAtt').click(function() {
    if($('.attIpClass').val() === undefined){
        alert("IP를 입력해주세요.");
        return false;
    }
    ...
    var dmgIpList = [];
    var checkedValues = '';
    var totalCnt = $('.attIpClass').length;
    ipCheckType = 'att'
    ...
    $.post(ctxPath + '/main/popup/comm/pAccDuplList.do', function(result) {
        HmWindow.open(pwin, '사고중복 확인', result, 500, 500, 'pAccDuplWindow_init', checkedValues);
    });
});
