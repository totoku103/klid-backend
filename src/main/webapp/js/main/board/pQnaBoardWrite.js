...
var Main = {
	...
	observe : function() {
        ...
        $('#fileUpload').on('uploadEnd', function(event) {
            if (--uploadCnt == 0) {
                location.href = $('#ctxPath').val() + '/main/board/pQnaBoardContents.do?boardNo=' + boardNo;
        ...
        $('#fileUpload').on('select', function(event) {
			...
			Server.get('/api/main/sys/getDetailBoardMgmtList', {
                data: { guid: '270241B8-AA1C-4BBA-948F-B9AF8BBD74DD'}, //문의의견 키값
	...
	initData : function() {
		...
		var sInstNm = $("#sInstName").val();
		$("#instNm").val(sInstNm);
	},

	saveContents : function() {
        var isSecret = 'N'; //비밀글 체크 여부
		/*if($('#isSecret').val()){
            isSecret = 'Y';
		}*/
		...
		var params={
            	...
            	groupItemNo: 0,	 //qna 그룹항목 없음
				userName: $("#sUserName").val()
		};
		Server.post('/api/main/sec/qnaBoard/addBoard', {
			data : params,
			success : function(result) {
