...
observe : function() {
	...
	$('#fileUpload').on('uploadEnd', function(event) {
		if (--uploadCnt == 0) {
			location.href = $('#ctxPath').val() + '/main/board/pResourceBoardContents.do?boardNo=' + boardNo;
	...
	$('#fileUpload').on('select', function(event) {
        ...
        Server.get('/api/main/sys/getDetailBoardMgmtList', {
            data: { guid: '35E56A6F-B4CF-4255-8300-A55BA44F7BA6'}, //보안자료실 설정 키값
...
console.log("zzzzz", linkVal)
			...
			var linkVal2 = linkVal.substring(start,linkVal.indexOf('&gt;'));
...
saveContents : function() {
	...
	var params={
			...
			userId: $('#sUserId').val(),
        	cateNo: $('#groupType').val(),
        	organCode: $('#sInstCd').val() //게시판 organCode 값
	};
	Server.post('/api/main/sec/resourceBoard/addBoard', {
		data : params,
		success : function(result) {
			...
			if ($('.jqx-file-upload-file-row').length == 0) {
				location.href = $('#ctxPath').val() + '/main/board/pResourceBoardContents.do?boardNo=' + result;
			} else {
				try{
