var $detailGrid, $date1, $date2, $time;
...
var Main = {
    ...
    initDesign: function() {
       ...
       //경과일
        var term_type_cd = new Array();
        $.ajax({
            type: 'GET',
            url: ctxPath + '/api/code/getCommonCode',
            data: {comCode1: '4012' , codeLvl: '2'},
            async: false,
            success: function (data) {
                term_type_cd.push({label: '전체', value: ''});
                $.each(data.resultData, function (index, data) {
                    term_type_cd.push({label: data.codeName, value: data.comCode2})
                });
            }
        });
        $('#term_type_cd').jqxDropDownList({source: term_type_cd, width: 140, height:20 ,autoDropDownHeight: true, selectedIndex : 0 })
...
//             columnsreorder:true,
        ...
        $detailGrid.on('rowdoubleclick', function (event) {
            var rowdata = HmGrid.getRowData($detailGrid, event.args.rowindex);
    ...
    exportExcel: function() {
        ...
        HmUtil.exportExcel(ctxPath + '/api/main/rpt/reportCollection/exportRetrieveIncidentDetail', params);
