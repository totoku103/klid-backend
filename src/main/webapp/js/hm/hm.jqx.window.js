var HmWindow = {
    create: function($pwin, pWidth, pHeight, modalZIndex) {
        ...
        if(modalZIndex === undefined) modalZIndex = 180;
    ...
    open: function($pwin, title, content, w, h, fnInit, fnInitParam) {
        ...
        $pwin.jqxWindow({ title: '<h1>' + title + '</h1>', content: content, position: 'center', resizable: true ,  modalZIndex: 250,
    ...
    popOpen: function($pwin, title, content, w, h, fnInit, fnInitParam, modalZIndex) {
        if(modalZIndex === undefined) modalZIndex = 180;
        if(w != null && h != null) {
            HmWindow.setSize($pwin, w, h);
        }
        $pwin.jqxWindow({ title: '<h1>' + title + '</h1>', content: content, position: 'center', resizable: true ,  modalZIndex: modalZIndex,
            initContent: function() {
                try {
                    if(fnInit == null) {
                        if(typeof pwindow_init === 'function') pwindow_init(fnInitParam);
