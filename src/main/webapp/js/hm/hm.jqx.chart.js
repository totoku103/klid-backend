var HmChart2 = {
	...
	getCommOptions: function(chartType, xUnit, yFormatFn) {
		if(chartType == this.T_LINE) {
			if($.isBlank(xUnit)) xUnit = 'day';
		}
		var opt = {
			title: null,
			description: null,
		...
		if(chartType == this.T_LINE) {
			$.extend(opt.xAxis, {
				...
				formatFunction: function(value) {
					var format = 'MM-dd';
                	switch(xUnit) {
                	case 'second': format = 'mm:ss'; break;
                	case 'minute': format = 'HH:mm'; break;
                	case 'hour': format = 'dd일 HH시'; break;
...
//                	visible: false,
	...
	toolTipFormatFn: function($chart, value, itemIndex, series, group, categoryValue, categoryAxis) {
		...
		var s = '<div style="text-align: left;"><b>' + $.format.date(categoryValue, 'yyyy-MM-dd HH:mm:ss') + '</b><br>';
	...
	getSeries: function(arrDataField, arrDisText, arrColor, isMarker) {
		...
		if($.isBlank(isMarker)) isMarker = false;
		var series = [];
...
//				   }
//				   else {
//					   return HmChart2.toolTipFormatFn($chart, value, itemIndex, series, group, categoryValue, categoryAxis);
//				   }
//			   },
			   series: series
			};
		}
		else if(seriesType == this.T_LINE)
		{
            return {
                type: seriesType,
                columnsGapPercent: 30,
...
//			   },
			...
			// AREA 타입일때 알파값 조정. 시리즈 가려지는 현상.
			if(seriesType == HmChart2.T_AREA) {
				$.each(series, function(idx, value) {
					value.opacity = 0.5;
				});
			}
