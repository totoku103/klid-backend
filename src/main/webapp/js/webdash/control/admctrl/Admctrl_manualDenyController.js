...
define(function (require) {
    var d3 = require('d3'),
        ...
        manualDenyHTML = require('text!./Admctrl_manualDenyHTML.jsp');

    function Admctrl_manualDenyController(id) {
        this.id = id || 'admctrl_manualDeny';
    }

    Admctrl_manualDenyController.prototype = {
        getHTML: function () {
        ...
        refresh: function (model) {
            ...
            this.svg.select("text#{0}_cnt1".substitute(this.id)).text(HmUtil.commaNum(data.receiptCnt));
            this.svg.select("text#{0}_cnt2".substitute(this.id)).text(HmUtil.commaNum(data.processCnt));
            this.svg.select("text#{0}_cnt3".substitute(this.id)).text(HmUtil.commaNum(data.completeCnt));
        },
    ...
    return Admctrl_manualDenyController;
});
