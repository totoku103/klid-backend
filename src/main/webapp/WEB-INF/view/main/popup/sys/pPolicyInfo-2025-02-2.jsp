...
<script>
    (function () {
        function onComparePolicyClickEvent(event) {
            ...
            const option = "width=1240,height=800,top=100,left=100,resizable=no,scrollbars=no,menubar=no,toolbar=no,location=no,status=no";
            window.open(url, 'compare-privacy-policy', option)
        }

        function onHistoryPolicyClick(event) {
            event.stopPropagation();
            const elementId = event.target.id;
            const url = ctxPath + '/main/popup/privacy-policy/' + elementId + '.do';
            const option = "width=1240,height=800,top=100,left=100,resizable=no,scrollbars=no,menubar=no,toolbar=no,location=no,status=no";
            window.open(url, 'privacy-policy-'  +elementId + '-popup', option);
        }
