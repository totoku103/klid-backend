...
<form id="p2Form">
    ...
    <div style="text-align:center; margin-top:10px;">
        <button id="p2btnSave" type="button" class="p_btnSave"></button>
...
<script>
    ...
    function p2window_init(rowdata) {
        ...
        $("#p2_menuName").val(rowdata.menuName);
        ...
        $('#p2_grpType').jqxDropDownList({ width: '95%', height: 21,
            ...
            $('#p2_grpType').val(rowdata.grpType)
        });
        ...
        $('#p2_condType').jqxDropDownList({ width: '95%', height: 21,
            ...
            Server.post('/api/engineer/popup/getLayoutMenuCond', {
                data: { guid: rowdata.guid},
                success: function(result) {

                    result.forEach(function (data, index) {
                        var items = $("#p2_condType").jqxDropDownList('getItems');
                        items.some(function (item, idx) {
                            if (item.value === data.condType) {
                                $("#p2_condType").jqxDropDownList('checkIndex', item.index);
                            }
    ...
    function p2window_save() {
        ...
        Server.post('/api/engineer/popup/saveLayoutMenuPage', {
            data: param,
            success: function(result) {
                alert('수정되었습니다.');
                pMain.search();
