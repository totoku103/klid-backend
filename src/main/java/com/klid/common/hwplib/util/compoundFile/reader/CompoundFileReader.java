...
public class CompoundFileReader {
	 ...
	 * @throws IOException
	 */
	public void moveChildStorage(String name) throws IOException {
		Entry e = currentStorage.getEntry(name);
		if (e != null && e.isDirectoryEntry()) {
			currentStorage = (DirectoryEntry) e;
		} else {
			throw new IOException("this is not storage.");
		}
	}
	...
	public StreamReader getChildStreamReader(String name, boolean compress,
			FileVersion fileVersion) throws NotSerializableException, IOException {
		Entry e = currentStorage.getEntry(name);
		if (e != null && e.isDocumentEntry()) {
			...
			throw new NotSerializableException("this is not stream.");
		}
	}
