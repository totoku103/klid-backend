...
import com.klid.api.logs.action.persistence.UserActionLogInstitutionMapper;
...
public class UserActionLogInstitutionService {

    private final InstitutionAggregateService institutionAggregateService;
    private final UserActionLogInstitutionMapper userActionLogInstitutionMapper;
    ...
    public List<InstitutionSummaryGridResDTO> getGridData(final ThirdPartySystemType systemType,
        ...
        if (ThirdPartySystemType.CTRS.equals(systemType)) {
            ...
            final List<InstitutionSummaryGridResDTO> institutionSummaryGridResDTOS =
                    userActionLogInstitutionMapper.selectCTRSSystemGridList(institutionCode, codeLevel, month);
            ...
            result = userActionLogInstitutionMapper.selectOtherSystemGridList(systemType.name(), institutionCode, month);
        }
    ...
    public InstitutionChartDTO getChatData(final ThirdPartySystemType systemType,
                                           ...
                                           final String month,
                                           final Integer codeLevel) {
        final List<Integer> chartXAxis = userActionLogInstitutionMapper.selectAllSystemChartXAxisList(month);
        final List<InstitutionChartYAxisDTO> chartData;

        if (ThirdPartySystemType.CTRS.equals(systemType)) {
            chartData = userActionLogInstitutionMapper.selectCTRSChartDataList(institutionCode, codeLevel, month);
        } else {
            chartData = userActionLogInstitutionMapper.selectOtherChartDataList(systemType.name(), institutionCode, month);
        }
