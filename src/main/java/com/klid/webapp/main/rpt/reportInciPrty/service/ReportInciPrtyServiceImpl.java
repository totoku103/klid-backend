...
import com.klid.webapp.main.rpt.reportInciPrty.persistence.ReportInciPrtyMapper;
...
public class ReportInciPrtyServiceImpl extends MsgService implements ReportInciPrtyService {


	public ReportInciPrtyServiceImpl(MessageSource messageSource, ReportInciPrtyMapper mapper) {
				super(messageSource);
		this.mapper = mapper;
	}
	...
	/** 사고유형 그리드 조회 */
	@Override
	public ReturnData getPrtyList(Criterion criterion) {
		return new ReturnData(mapper.selectInciPrtyList(criterion.getCondition()));
	}

	@Override
	public ReturnData exportReportInciPrty(Criterion criterion) {
		try{
			...
			ArrayList<LinkedHashMap<String,Object>> prtyGrid = (ArrayList<LinkedHashMap<String,Object>>)reqMap.get("prtyGrid");

			String filename = AppGlobal.reportTemplate+"report_type_"+prtyGrid.size()+".hwp";
			...
			rows.get(0).getCellList().get(0).getParagraphList().getParagraph(0).getText().addString("우선순위보고서");
			...
			rows.get(0).getCellList().get(0).getParagraphList().getParagraph(0).createText();
			rows.get(0).getCellList().get(0).getParagraphList().getParagraph(0).getText().addString("우선순위");
			...
			for(int i=1; i<rows.size(); i++){
				...
				rows.get(i).getCellList().get(0).getParagraphList().getParagraph(0).getText().addString((String)prtyGrid.get(j).get("name"));
				rows.get(i).getCellList().get(1).getParagraphList().getParagraph(0).createText();
				rows.get(i).getCellList().get(1).getParagraphList().getParagraph(0).getText().addString(String.valueOf(prtyGrid.get(j).get("y")));
			}
			...
			resultMap.put("fileName", today+"_inci_Prty_report");
