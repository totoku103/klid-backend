...
public class ReportInciTypeServiceImpl extends MsgService implements ReportInciTypeService {
	...
	/** 사고유형 그리드 조회 */
	@Override
	public ReturnData getTypeList(Criterion criterion) {
		return new ReturnData(mapper.selectInciTypeList(criterion.getCondition()));
	}

	@Override
	public ReturnData exportReportInciType(Criterion criterion) {
		try{
			...
			ArrayList<LinkedHashMap<String,Object>> typeGrid = (ArrayList<LinkedHashMap<String,Object>>)reqMap.get("typeGrid");

			String filename = AppGlobal.reportTemplate+"report_type_"+typeGrid.size()+".hwp";
			...
			rows.get(0).getCellList().get(0).getParagraphList().getParagraph(0).getText().addString("사고유형보고서");
			...
			rows.get(0).getCellList().get(0).getParagraphList().getParagraph(0).createText();
			rows.get(0).getCellList().get(0).getParagraphList().getParagraph(0).getText().addString("사고유형");
			...
			for(int i=1; i<rows.size(); i++){
				...
				rows.get(i).getCellList().get(0).getParagraphList().getParagraph(0).getText().addString((String)typeGrid.get(j).get("name"));
				rows.get(i).getCellList().get(1).getParagraphList().getParagraph(0).createText();
				rows.get(i).getCellList().get(1).getParagraphList().getParagraph(0).getText().addString(String.valueOf(typeGrid.get(j).get("y")));
			}
			...
			resultMap.put("fileName", today+"_type_report");
